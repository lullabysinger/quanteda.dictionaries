...
#' MODIFIED TO ENABLE REGEX AND CASE SENSITIVE SEARCH
#'
...
#' @examples
#' liwcahead(data_char_testphrases)
#'
...
#'                                   mwe = "out of"))
#' liwcahead(txt, dict, what = "word")
#' liwcahead(txt, dict, what = "fasterword")
...
#'              format = "LIWC")
#' dat_liwc_analysis <- liwcahead(data_corpus_inaugural, dict_liwc_2015)
...
#' @export
#' @import quanteda
liwcahead <- function(x, ...) {
  UseMethod("liwcahead")
}

#' @rdname liwcahead
#' @export
liwcahead <- function(x, dictionary = NULL, tolower = TRUE, verbose = TRUE, regex = FALSE, digits = 2, ...) {
  ...
  # lower case the texts if required -- only for regular globmode
  if (tolower && !regex)
  ...
  if (regex) {
    if (!is.null(dictionary)) toks <- tokens_lookup(toks, dictionary, valuetype="regex")
    warning("Regex mode on for liwcahead -- this is still experimental.")
